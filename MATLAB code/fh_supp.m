...
for n=1:N_realization
  fprintf('[D\t n=%d]\n',n);
  for i_snr = 1:length(SNRdB_list_P2)
    SNRdB = SNRdB_list_P2(i_snr);
...
y_SINR_P2 = mean(y_SINR,2);
y_SINR_mean_P2 = mean(y_SINR_mean,2);
yDet_BER_min_P2 = mean(yDet_BER_min,2);
yDet_BER_p80_P2 = mean(yDet_BER_p80,2);
yDet_BER_p90_P2 = mean(yDet_BER_p90,2);
yDet_BER_max_P2 = mean(yDet_BER_max,2);
...
y2_SINR_P2 = mean(y2_SINR,2);
y2_SINR_mean_P2 = mean(y2_SINR_mean,2);
yRand_BER_min_P2 = mean(yRand_BER_min,2);
yRand_BER_max_P2 = mean(yRand_BER_max,2);
yRand_BER_p10_P2 = mean(yRand_BER_p10,2)
yRand_BER_p20_P2 = mean(yRand_BER_p20,2)
yRand_BER_p30_P2 = mean(yRand_BER_p30,2)
yRand_BER_p50_P2 = mean(yRand_BER_p50,2)
yRand_BER_p80_P2 = mean(yRand_BER_p80,2)

% fprintf("output min-SINR for Random Jamming case:\n");
% disp(y2_SINR)

fprintf("------------- End Data Generation --------------------------\n")
save fh_supp2.mat y2_SINR_P2 y2_SINR_mean_P2 yRand_BER_min_P2 yRand_BER_max_P2 yRand_BER_p10_P2 yRand_BER_p20_P2 yRand_BER_p30_P2 yRand_BER_p50_P2 yRand_BER_p80_P2 y_SINR_P2 y_SINR_mean_P2 yDet_BER_min_P2 
%genFigures;
...
function [output] = simBER_givenYiYjam(Yi, Yjam, snrdB)
  ...
  Nh = size(Yjam,2);
  Nu = length(Yi);
  Nsym = 5e2; % Number of symbols transmitted per FH interval 
